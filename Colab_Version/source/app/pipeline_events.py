...
class DownloadPresentation:
    ...
    def consume(self, event: dict[str, Any]) -> None:
        ...
        changed_job = any(
            event.get(key) is not None and event.get(key) != row.get(key)
            for key in ("filename", "document_id", "document_attempt")
        )
        if changed_job or kind == "worker_started":
            row.clear()
            row.update(worker=worker_id, state="STARTING", bytes_downloaded=0, total_bytes=0)
        for key in (
        ...
        if int(row.get("bytes_downloaded", 0) or 0) > int(row.get("total_bytes", 0) or 0):
            # A stale or inconsistent server total is not a usable denominator.
            row["total_bytes"] = 0
