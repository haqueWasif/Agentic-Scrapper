...
from urllib.parse import parse_qs, urlparse, parse_qsl, urlencode, urlunparse
...
def stable_document_id(candidate: dict[str, Any]) -> str:
    ...
    parsed = urlparse(source_url or (sorted(mirrors)[0] if mirrors else ''))
    canonical = urlunparse((parsed.scheme.lower(), parsed.netloc.lower(), parsed.path, '',
                           urlencode(sorted(parse_qsl(parsed.query))), ''))
    identity = "\n".join((canonical, ' '.join(str(candidate.get('title') or '').casefold().split())))
